...
class BuilderImputation extends ContainerAware
{
    /** ************************************************************************
    ...
    protected function addChildList(MenuItem $menu) {
        $menu->addChild('List', array('route' => 'finance_operation_imputation_home'))->setAttribute('icon', 'glyphicon-list');
    }
    ...
    protected function addChildNew(MenuItem $menu) {
        $menu->addChild('New Imputation', array('route' => 'finance_operation_imputation_add'))->setAttribute('icon', 'glyphicon-plus');
    }
    ...
    protected function addChildModify(MenuItem $menu, array $options) {
        $menu->addChild('Modify', array(
                    'route' => 'finance_operation_imputation_modify', 
                    'routeParameters' => array('imputation_id' => $options['imputation']->getId())))
    ...
    protected function addChildDelete(MenuItem $menu, array $options) {
        $menu->addChild('Delete', array('uri' => '#'))
            ...
            ->setLinkAttribute('data-href', $this->container->get('router')->generate('finance_operation_imputation_delete', array('imputation_id' => $options['imputation']->getId())))
            ;
    }
