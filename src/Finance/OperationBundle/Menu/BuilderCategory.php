...
class BuilderCategory extends ContainerAware
{
    /** ************************************************************************
    ...
    protected function addChildList(MenuItem $menu) {
        $menu->addChild('List', array('route' => 'finance_operation_category_home'))->setAttribute('icon', 'glyphicon-list');
    }
    ...
    protected function addChildNew(MenuItem $menu) {
        $menu->addChild('New Category', array('route' => 'finance_operation_category_add'))->setAttribute('icon', 'glyphicon-plus');
    }
    ...
    protected function addChildModify(MenuItem $menu, array $options) {
        $menu->addChild('Modify', array(
                    'route' => 'finance_operation_category_modify', 
                    'routeParameters' => array('category_id' => $options['category']->getId())))
    ...
    protected function addChildDelete(MenuItem $menu, array $options) {
        $menu->addChild('Delete', array('uri' => '#'))
            ...
            ->setLinkAttribute('data-href', $this->container->get('router')->generate('finance_operation_category_delete', array('category_id' => $options['category']->getId())))
            ;
    }
