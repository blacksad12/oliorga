...
class BuilderShoplist extends ContainerAware
{
    /** ************************************************************************
    ...
    protected function addChildList(MenuItem $menu) {
        $menu->addChild('List', array('route' => 'nutri_recipe_shoplist_home'))->setAttribute('icon', 'glyphicon-list');
    }
    ...
    protected function addChildNew(MenuItem $menu) {
        $menu->addChild('New Shoplist', array('route' => 'nutri_recipe_shoplist_add'))->setAttribute('icon', 'glyphicon-plus');
    }
    ...
    protected function addChildModify(MenuItem $menu, array $options) {
        $menu->addChild('Modify', array(
                    'route' => 'nutri_recipe_shoplist_modify', 
                    'routeParameters' => array('shoplist_id' => $options['shoplist']->getId())))
    ...
    protected function addChildDelete(MenuItem $menu, array $options) {
        $menu->addChild('Delete', array('uri' => '#'))
            ...
            ->setLinkAttribute('data-href', $this->container->get('router')->generate('nutri_recipe_shoplist_delete', array('shoplist_id' => $options['shoplist']->getId())))
            ;
    }
