...
class BuilderMenu extends ContainerAware
{
    ...
    protected function addChildList(MenuItem $menu) {
        $menu->addChild('List', array('route' => 'nutri_recipe_menu_home'))->setAttribute('icon', 'glyphicon-list');
    }
    ...
    protected function addChildNew(MenuItem $menu) {
        $menu->addChild('New Menu', array('route' => 'nutri_recipe_menu_add'))->setAttribute('icon', 'glyphicon-plus');
    }
    ...
    protected function addChildModify(MenuItem $menu, array $options) {
        $menu->addChild('Modify', array(
                    'route' => 'nutri_recipe_menu_modify', 
                    'routeParameters' => array('menu_id' => $options['menu']->getId())))
    ...
    protected function addChildDelete(MenuItem $menu, array $options) {
        $menu->addChild('Delete', array('uri' => '#'))
            ...
            ->setLinkAttribute('data-href', $this->container->get('router')->generate('nutri_recipe_menu_delete', array('menu_id' => $options['menu']->getId())))
            ;
    }
